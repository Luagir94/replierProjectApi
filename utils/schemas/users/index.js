...
const validateCreateUser = (data) => {
  const schema = Joi.object({
    ...
    role: Joi.string().trim(true).required().messages({
  ...
  return (schema.validate({ name: data.name, lastName: data.lastName, userName: data.userName, role : data.role }))
}
...
const validatePassword = (data) => {
  const schema = Joi.object({
    newPassowrd: Joi.string().min(6).trim(true).messages({
  ...
  return (schema.validate({ newPassowrd: data.newPassowrd }))
}
